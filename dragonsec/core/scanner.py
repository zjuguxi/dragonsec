...
class SecurityScanner:
    def __init__(
        self,
        mode: ScanMode = ScanMode.SEMGREP_ONLY,
        api_key: str = None,
        verbose: bool = False,
    ...
    def _should_scan_file(self, file_path: str) -> bool:
        ...
        if ext and ext not in self.supported_extensions:
            if self.verbose:
                logger.debug(
                    f"Skipping file with unsupported extension: {file_path} (ext: {ext})"
                )
            return False
        ...
        try:
            with open(file_path, "r", encoding="utf-8") as f:
                try:
                    ...
                    if self.verbose:
                        logger.debug(
                            f"Skipping binary file (decode error): {file_path}"
                        )
                    return False
        ...
        if not self.include_tests:
            ...
            for pattern in self.test_dir_patterns:
                if pattern in file_path and "fixtures" not in file_path:
                    if self.verbose:
                        logger.debug(
                            f"Skipping test file in test directory: {file_path}"
                        )
                    return False
            ...
            for pattern in self.test_file_patterns:
                if pattern in file_name and "fixtures" not in file_path:
                    if self.verbose:
                        logger.debug(
                            f"Skipping test file with test pattern in name: {file_path}"
                        )
                    return False
    ...
    async def scan_file(self, file_path: str, progress_bar=None) -> Dict:
        ...
        try:
            ...
            if self.mode == ScanMode.SEMGREP_ONLY:
                ...
                if not self.ai_provider:
                    ...
                    return {
                        ...
                        "metadata": {"error": "AI provider not initialized"},
                    }
                ...
                ai_result = await self.ai_provider.analyze_code(
                    content, file_path, context
                )

                if self.verbose:
                    logger.debug(
                        f"AI result: {json.dumps(ai_result, indent=2)[:500]}..."
                    )
                    logger.debug(
                        f"Found {len(ai_result.get('vulnerabilities', []))} vulnerabilities"
                    )
            ...
            if self.verbose:
                ...
                print(
                    f"🔎 Found {len(result.get('vulnerabilities', []))} potential issues"
                )
    ...
    async def scan_directory(self, path: str, mode: str = None) -> Dict:
        ...
        try:
            path = Path(path)
            self.file_context.set_scan_root(
            ...
            if self.verbose:
                ...
                )  # Always show this even in non-verbose mode
            ...
            files_to_scan = []
            skipped_files = 0
            skipped_reasons = {"extension": 0, "directory": 0, "test_file": 0}
            ...
            if self.verbose:
                logger.info(
                    f"Found {len(files_to_scan)} files to scan, {skipped_files} files skipped"
                )
            else:
                print(
                    f"Found {len(files_to_scan)} files to scan, {skipped_files} files skipped"
            ...
            for i in range(0, len(files_to_scan), self.batch_size):
                ...
                print(
                    f"Batch {i//self.batch_size + 1}/{(len(files_to_scan) + self.batch_size - 1)//self.batch_size}: {batch_info}"
                )
            ...
            try:
                ...
                for result in results:
                    if isinstance(result, dict) and "vulnerabilities" in result:
                        summary["vulnerabilities"].extend(
            ...
            if self.ai_provider and hasattr(self.ai_provider, "filter_false_positives"):
                try:
                    ...
                    # 过滤误报
                    filtered_summary = await self.ai_provider.filter_false_positives(
                    ...
                    logger.info(
                        f"Filtered false positives: {summary['metadata'].get('original_vulnerabilities', 0)} -> {summary['metadata'].get('filtered_vulnerabilities', len(summary.get('vulnerabilities', [])))}"
                    )

                    # 对过滤后的结果进行深度分析
                    if self.ai_provider and hasattr(
                        self.ai_provider, "deep_audit_vulnerabilities"
                    ):
                        try:
                            ...
                            # 深度分析
                            audited_summary = (
                                await self.ai_provider.deep_audit_vulnerabilities(
                                    summary, file_contents
                                )
                            )
                            ...
                            if "metadata" in summary and summary["metadata"].get(
                                "deep_audited"
                            ):
                                logger.info(
                                    f"Audit timestamp: {summary['metadata'].get('audit_timestamp')}"
                                )
                        except Exception as e:
            ...
            if self.output_dir:
                ...
                output_file = os.path.join(
                    self.output_dir, f"{path_name}_{scan_mode}_{timestamp}.json"
                )
    ...
    def _summarize_results(self, results: List[Dict]) -> Dict:
        ...
        if self.mode == ScanMode.SEMGREP_ONLY:
            # 仅使用 semgrep
            summary_text = (
                f"Found {len(total_vulns)} vulnerabilities from semgrep analysis"
            )
        elif self.mode in [
            ScanMode.OPENAI,
            ScanMode.GEMINI,
    ...
    def _save_scan_results(self, result: Dict, file_path: str) -> str:
        ...
        try:
            ...
            timestamp = time.strftime("%Y%m%d_%H%M%S")
            output_file = os.path.join(
                self.output_dir, f"{file_name}_{self.mode.value}_{timestamp}.json"
            )
    ...
    def add_arguments(parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument(
            "--batch-delay", type=float, default=0.1, help="Delay between batches"
        )
            ...
            if "output_file" in metadata:
                ...
                try:
                    ...
                    if json_files:
                        # 按修改时间排序，获取最新的文件
                        latest_file = sorted(
                            json_files, key=lambda x: x[1], reverse=True
                        )[0][0]
