...
class SemgrepRunner:
                ...
                finding = {
                    ...
                    "extra": {
                        ...
                        "metadata": {
                            ...
                            "fix": rule.get(
                                "fix",
                                "Review and fix according to secure coding guidelines",
                            ),
                        },
    ...
    async def run_scan(self, target_path: str) -> Dict:
        ...
        cmd = [
            ...
            "--json",
            "--timeout",
            "60",  # 增加单个规则的超时时间到 60 秒
            "--timeout-threshold",
            "3",  # 允许更多规则超时
            "--jobs",
            str(min(2, self.workers)),  # 限制并行数
            "--max-memory",
            "512",  # 内存限制（MB）
            "--optimizations",
            "all",
            "--exclude",
            "node_modules,build,dist,*.min.js,venv",
        ...
        try:
            ...
            try:
                stdout_bytes, stderr_bytes = await asyncio.wait_for(
                    process.communicate(), timeout=120.0  # 增加整体超时时间到 120 秒
                )
    ...
    def format_results(self, results: Dict) -> List[Dict]:
        ...
        for finding in results.get("results", []):
            ...
            vuln = {
                ...
                "severity": self._convert_severity(
