...
Your task is to identify security issues, assess their severity, and provide recommendations."""
    ...
    async def _call_api(self, prompt: str) -> str:
        ...
        try:
            ...
            if response.status_code != 200:
                logger.error(f"Error calling local model API: {response.status_code} {response.text}")
                raise Exception(f"Error calling local model API: {response.status_code} {response.text}")
    ...
    def _should_skip_file(self, file_path: str) -> bool:
        ...
        # 跳过非代码文件
        non_code_files = ['LICENSE', 'README.md', 'PKG-INFO', 'MANIFEST.in', 'requirements.txt']
        if file_name in non_code_files:
            logger.info(f"Skipping non-code file: {file_path}")
            return True
        
        # 跳过某些扩展名的文件
        non_code_exts = ['.md', '.txt', '.rst', '.json', '.yaml', '.yml', '.toml', '.ini']
    ...
    def _post_process_vulnerabilities(self, vulnerabilities: List[Dict], file_path: str) -> List[Dict]:
        ...
        for vuln in vulnerabilities:
            ...
            if not self._is_english(description):
                logger.warning(f"Skipping non-English vulnerability: {description[:50]}...")
                continue
            ...
            if "file_name" not in vuln and file_path:
                vuln["file_name"] = os.path.basename(file_path) if file_path else "unknown"
...
"""
    ...
    def _parse_response(self, response: str, file_path: str = None) -> Dict:
        ...
        try:
            ...
            # 提取漏洞类型
            vuln_types = re.findall(r'(?:vulnerability|issue)(?:\s+type)?[:\s]+([A-Za-z\s_]+)', response, re.IGNORECASE)
            ...
            if vulnerabilities:
                logger.debug(f"Extracted {len(vulnerabilities)} vulnerabilities using regex")
                return {
    ...
    def _validate_result(self, result: Dict, file_path: str) -> Dict:
        ...
        # 处理漏洞列表
        result["vulnerabilities"] = self._post_process_vulnerabilities(result["vulnerabilities"], file_path)
    ...
    def _analyze_text_response(self, response: str, file_path: str = None) -> Dict:
        ...
        # 尝试提取漏洞信息
        # 1. 寻找明确的漏洞标记
        vuln_sections = re.findall(r'(?:vulnerability|issue|security\s+problem)[:\s]+(.*?)(?:\n\n|\Z)', response, re.IGNORECASE | re.DOTALL)
        
        # 2. 寻找常见漏洞类型
        common_vulns = [
            "SQL Injection", "XSS", "CSRF", "Command Injection", "Path Traversal",
            "Insecure Deserialization", "XXE", "SSRF", "Open Redirect", "Insecure Direct Object Reference",
        ...
        for vuln_type in common_vulns:
            if re.search(rf'\b{re.escape(vuln_type)}\b', response, re.IGNORECASE):
                # 尝试提取相关描述
                desc_match = re.search(rf'\b{re.escape(vuln_type)}\b[^.]*\.', response, re.IGNORECASE)
                description = desc_match.group(0) if desc_match else f"Potential {vuln_type} vulnerability detected"
        ...
        if vulnerabilities:
            ...
            summary = f"Found {len(vulnerabilities)} potential vulnerabilities in the code."
        else:
            # 检查是否有明确的安全评价
            if re.search(r'no\s+vulnerabilities', response, re.IGNORECASE) or \
               re.search(r'code\s+is\s+secure', response, re.IGNORECASE):
                security_score = 100
    ...
    def _fix_json_format(self, json_str: str) -> str:
        ...
        for i in reversed(open_quotes):
            ...
            # 找到最近的分隔符
            delimiters = [d for d in [next_delimiter, next_brace, next_bracket] if d != -1]
            if delimiters:
        ...
        try:
            ...
            if "Expecting property name enclosed in double quotes" in error_msg and error_pos is not None:
                # 在错误位置前查找最近的非空白字符
                i = error_pos - 1
                    ...
                    if json_str[i] in '{,':
                        ...
                        if j < len(json_str):
                            ...
                            if json_str[j].isalpha() or json_str[j] == '_':
                                # 查找属性名的结束位置
                                k = j
                                while k < len(json_str) and (json_str[k].isalnum() or json_str[k] == '_'):
                                    k += 1
                                
                                # 在属性名周围添加引号
                                property_name = json_str[j:k]
                                json_str = json_str[:j] + '"' + property_name + '"' + json_str[k:]
            ...
            try:
                ...
                try:
                    ...
                    if start != -1 and end != -1 and start < end:
                        ...
                        # 替换所有未引用的属性名
                        json_obj = re.sub(r'([{,]\s*)([a-zA-Z_][a-zA-Z0-9_]*)\s*:', r'\1"\2":', json_obj)
